...
import { getTeam, toFixtureCard, type ApiTeamDetail } from "./api";
...
export function TeamProfilePage({ slug, onBack, onOpenMatch, onOpenCompetition }: Props) {
  ...
  return <div style={{ minHeight: "100%", paddingBottom: 32 }}>
    ...
    <section style={{ margin: "24px 20px 0" }}><h2 style={{ fontSize: 15, margin: "0 0 10px" }}>League positions</h2>{detail.standings.length === 0 ? <EmptyState title="No league position is available." /> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 8 }}>{detail.standings.map((row) => <button key={`${row.competition.slug}-${row.position}`} type="button" onClick={() => onOpenCompetition(row.competition.slug)} style={{ background: "#12121a", border: "1px solid rgba(255,255,255,0.07)", color: "#ececf1", textAlign: "left", borderRadius: 10, padding: 12, cursor: "pointer" }}><div style={{ display: "flex", alignItems: "center", gap: 8 }}><Crest src={row.competition.logo_url} name={row.competition.name} size={24} /><strong style={{ flex: 1, fontSize: 13 }}>{row.competition.name}</strong><span style={{ color: "#dc2626", fontSize: 18, fontWeight: 900 }}>#{row.position}</span></div><small style={{ display: "block", marginTop: 8, color: "#8b8b9a" }}>{row.points} points · {row.played} played</small></button>)}</div>}</section>
  </div>;
}
