...
import { getCompetitions, getMatches, teamLogoSources, competitionLogoSources, type ApiCompetition, type ApiMatch } from "./api";
...
export function SearchPage({ setActiveScreen, onOpenMatch, onOpenCompetition, onOpenTeam }: Props) {
  ...
  return <div style={{ minHeight: "100%", paddingBottom: 32 }}>
    ...
    {loading ? <EmptyState title="Loading searchable football data…" /> : error ? <ErrorState message="Could not load search data." onRetry={load} /> : !term ? <EmptyState title="Search the matches and competitions we currently have." detail="Search for clubs, leagues, or countries." /> : <div style={{ padding: "18px 20px" }}>
      <ResultSection title="Matches" empty="No matching matches." >{matchResults.map((match) => {
        const homeSrcs = match.home_team ? teamLogoSources({ logo_url: match.home_team.logo_url, provider_team_id: match.home_team.provider_team_id, provider_name: match.home_team.provider_name }) : [];
        return <button key={match.id} type="button" onClick={() => onOpenMatch(match.id)} style={resultButton}><Crest src={homeSrcs[0]} fallbackSrcs={homeSrcs.slice(1)} name={match.home_team?.name} abbr={match.home_team?.abbr} size={26} /><span style={{ flex: 1, textAlign: "left", minWidth: 0 }}><strong style={{ display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{match.home_team?.name || "TBD"} vs {match.away_team?.name || "TBD"}</strong><small style={{ color: "#8b8b9a" }}>{match.competition?.name || match.league || "Football"}</small></span><strong style={{ fontVariantNumeric: "tabular-nums" }}>{match.home_score}–{match.away_score}</strong><small style={{ color: match.status === "live" ? "#dc2626" : "#8b8b9a", fontWeight: 800 }}>{match.status === "live" ? `${match.minute ?? ""}'` : match.status === "finished" ? "FT" : "Scheduled"}</small></button>;
      })}</ResultSection>
      <ResultSection title="Competitions" empty="No matching competitions." >{competitionResults.map((competition) => {
        const compSrcs = competitionLogoSources({ logo_url: competition.logo_url, provider_competition_id: competition.provider_competition_id, provider_name: competition.provider_name });
        return <button key={competition.slug} type="button" onClick={() => onOpenCompetition(competition.slug)} style={resultButton}><Crest src={compSrcs[0]} fallbackSrcs={compSrcs.slice(1)} name={competition.name} size={26} /><span style={{ flex: 1, textAlign: "left" }}><strong style={{ display: "block" }}>{competition.name}</strong><small style={{ color: "#8b8b9a" }}>{competition.current_season || "View standings"}</small></span></button>;
      })}</ResultSection>
      <ResultSection title="Teams in available fixtures" empty="No matching teams in the current fixture data." >{teams.map((team) => {
        const teamSrcs = teamLogoSources({ logo_url: team.logo_url, provider_team_id: team.provider_team_id, provider_name: team.provider_name });
        return <button key={team.slug || team.name} type="button" onClick={() => team.slug && onOpenTeam(team.slug)} style={resultButton}><Crest src={teamSrcs[0]} fallbackSrcs={teamSrcs.slice(1)} name={team.name} abbr={team.abbr} size={26} /><span style={{ flex: 1, textAlign: "left" }}><strong style={{ display: "block" }}>{team.name}</strong><small style={{ color: "#8b8b9a" }}>Open team page</small></span></button>;
      })}</ResultSection>
    </div>}
  </div>;
}
