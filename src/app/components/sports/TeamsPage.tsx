...
import { getTeams, type ApiTeam } from "./api";
...
export function TeamsPage({ setActiveScreen, onOpenTeam }: Props) {
  ...
  return <div style={{ minHeight: "100%", paddingBottom: 32 }}>
    ...
    {loading ? <EmptyState title="Loading teams…" /> : error ? <ErrorState message="Could not load teams." onRetry={load} /> : visible.length === 0 ? <EmptyState title="No teams match your search." /> : <div style={{ padding: "0 20px", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(210px, 1fr))", gap: 8 }}>{visible.map((team) => <button key={team.slug} type="button" onClick={() => onOpenTeam(team.slug)} style={{ display: "flex", alignItems: "center", gap: 10, textAlign: "left", padding: 11, borderRadius: 10, background: "#12121a", border: "1px solid rgba(255,255,255,0.07)", color: "#ececf1", cursor: "pointer" }}><Crest src={team.logo_url} name={team.name} abbr={team.abbr} size={32} /><span style={{ minWidth: 0 }}><strong style={{ display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: 13 }}>{team.name}</strong>{team.country && <small style={{ color: "#8b8b9a" }}>{team.country}</small>}</span></button>)}</div>}
  </div>;
}
